...
import { FaHeart, FaRegHeart, FaStar, FaPlus, FaArrowLeft } from "react-icons/fa"; 
...
export default function ShoeDetail() {
  ...
  // --- 1. INSTANT STATE INITIALIZATION ---
  ...
  // --- HELPER NOTIFIKASI (AUTO HIDE 3 DETIK) ---
  useEffect(() => {
    if (notification) {
      ...
      // Cleanup: Reset timer jika ada notif baru sebelum 3 detik habis
    ...
    const syncData = async () => {
      ...
      try {
        ...
        if (userId && freshData.shoe_id) {
            sendInteraction(userId, freshData.shoe_id, 'click', 1).catch(e => console.warn(e));
        }
      } catch (err) {
  ...
  const handleReviewSubmit = async (e) => {
    ...
    const updatedShoeData = { ...shoeData, rating: Number(updatedAverage.toFixed(1)), reviews: [addedReview, ...currentReviews] };
    setShoeData(updatedShoeData);
    setNewReview({ rating: 0, text: "" }); 
    localStorage.setItem(`shoe_detail_${slug}`, JSON.stringify(updatedShoeData));

    const updatedGlobalList = allShoes.map(s => s.shoe_id === shoeData.shoe_id ? { ...s, rating: updatedShoeData.rating } : s);
    updateShoeState(updatedGlobalList);

    try {
      await api.post("/api/add-review/", { shoe_id: shoeData.shoe_id, rating: newReview.rating, text: newReview.text }, { headers: { Authorization: `Token ${token}` } });
      if (userId) {
  ...
  const handleToggleFavorite = async () => {
    ...
    try {
      await api.post("/api/favorites/toggle/", { shoe_id: String(shoeData.shoe_id) }, { headers: { Authorization: `Token ${token}` } });
      if (userId) sendInteraction(userId, shoeData.shoe_id, 'like', interactionValue).catch(console.warn);
    } catch (err) {
  ...
  const handleAddCompare = () => {
    ...
    if (compareList.some((item) => item.shoe_id === shoeData.shoe_id)) {
      showNotification(`"${shoeData.name || shoeData.model}" is already in list.`);
      return;
    }
    if (compareList.length >= 5) {
      showNotification("Compare list is full (Max 5).");
      return;
    }
  ...
  return (
    <div className="min-h-screen bg-[#4a76a8]">
      <Navbar />
      <div className="container mx-auto px-4 md:px-14 pt-35 pb-12 relative">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-8">
                  ...
                  <div className="space-y-5 lg:border-l lg:border-gray-200 lg:pl-6">
                    ...
                    <div>
                        ...
                        <p className="text-gray-800 font-bold">{shoeData.arch_neutral ? "Neutral" : shoeData.arch_stability ? "Stability" : "-"}</p>
                    </div>
                    <div>
                  ...
                  <div className="space-y-5 lg:border-l lg:border-gray-200 lg:pl-6">
                    ...
                    <div>
                        ...
                        <p className="text-gray-800 font-bold">{shoeData.stiffness_scaled === 1 ? "Flexible" : shoeData.stiffness_scaled === 3 ? "Moderate" : shoeData.stiffness_scaled === 5 ? "Stiff" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Heel Counter Stiffness</p>
                        <p className="text-gray-800 font-bold">{shoeData.heel_stiff === 1 ? "Flexible" : shoeData.heel_stiff === 3 ? "Moderate" : shoeData.heel_stiff === 5 ? "Stiff" : "-"}</p>
                    </div>
                    {isTrail && (
                        <div>
                            <p className="text-gray-400 text-xs font-bold">Shock Absorption</p>
                            <p className="text-gray-800 font-bold">{shoeData.shock_absorption === 1 ? "Low" : shoeData.shock_absorption === 3 ? "Moderate" : shoeData.shock_absorption === 5 ? "High" : "-"}</p>
                        </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-5 lg:border-l lg:border-gray-200 lg:pl-6">
                    <div>
                        ...
                        <p className="text-gray-800 font-bold">{shoeData.midsole_softness === 1 ? "Firm" : shoeData.midsole_softness === 3 ? "Balance" : shoeData.midsole_softness === 5 ? "Soft" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Torsional Rigidity</p>
                        <p className="text-gray-800 font-bold">{shoeData.torsional_rigidity === 1 ? "Flexible" : shoeData.torsional_rigidity === 3 ? "Moderate" : shoeData.torsional_rigidity === 5 ? "Stiff" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Breathability</p>
                        <p className="text-gray-800 font-bold">{shoeData.breathability_scaled === 1 ? "Warm" : shoeData.breathability_scaled === 3 ? "Moderate" : shoeData.breathability_scaled === 4 ? "Good" : shoeData.breathability_scaled === 5 ? "Breathable" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Toebox Durability</p>
                        <p className="text-gray-800 font-bold">{shoeData.toebox_durability === 1 ? "Very Bad" : shoeData.toebox_durability === 2 ? "Bad" : shoeData.toebox_durability === 3 ? "Decent" : shoeData.toebox_durability === 4 ? "Good" : shoeData.toebox_durability === 5 ? "Very Good" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Heel Padding Durability</p>
                        <p className="text-gray-800 font-bold">{shoeData.heel_durability === 1 ? "Very Bad" : shoeData.heel_durability === 2 ? "Bad" : shoeData.heel_durability === 3 ? "Decent" : shoeData.heel_durability === 4 ? "Good" : shoeData.heel_durability === 5 ? "Very Good" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Outsole Durability</p>
                        <p className="text-gray-800 font-bold">{shoeData.outsole_durability === 1 ? "Very Bad" : shoeData.outsole_durability === 2 ? "Bad" : shoeData.outsole_durability === 3 ? "Decent" : shoeData.outsole_durability === 4 ? "Good" : shoeData.outsole_durability === 5 ? "Very Good" : "-"}</p>
                    </div>
                    {isTrail && (
                        <div>
                            <p className="text-gray-400 text-xs font-bold">Energy Return</p>
                            <p className="text-gray-800 font-bold">{shoeData.energy_return === 1 ? "Low" : shoeData.energy_return === 3 ? "Moderate" : shoeData.energy_return === 5 ? "High" : "-"}</p>
                        </div>
                    )}
                  </div>
                  ...
                  <div className="space-y-5 lg:border-l lg:border-gray-200 lg:pl-6">
                    ...
                    <div>
                        ...
                        <p className="text-gray-800 font-bold">{shoeData.width_fit === 1 ? "Narrow" : shoeData.width_fit === 3 ? "Medium" : shoeData.width_fit === 5 ? "Wide" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Toebox Width</p>
                        <p className="text-gray-800 font-bold">{shoeData.toebox_width === 1 ? "Narrow" : shoeData.toebox_width === 3 ? "Medium" : shoeData.toebox_width === 5 ? "Wide" : "-"}</p>
                    </div>
                    <div>
                        <p className="text-gray-400 text-xs font-bold uppercase tracking-wide">Season</p>
                        <p className="text-gray-800 font-bold">{shoeData.season_all ? "All Season" : shoeData.season_summer ? "Summer" : "Winter"}</p>
                    </div>
                    {isTrail && (
                      <div className="space-y-5">
                        ...
                        <div>
                            ...
                            <p className="text-gray-800 font-bold">{shoeData.traction_scaled === 1 ? "Low" : shoeData.traction_scaled === 3 ? "Moderate" : shoeData.traction_scaled === 5 ? "High" : "-"}</p>
                        </div>
                        <div>
          ...
          <div className="bg-orange-400 rounded-t-2xl p-4 flex items-center justify-between">
            <div className="flex items-center space-x-4">
              ...
              <div className="flex text-2xl text-white">
                {[1, 2, 3, 4, 5].map((star) => (<FaStar key={star} className={`cursor-pointer ${star <= newReview.rating ? "text-yellow-300" : "text-gray-200"}`} onClick={() => setNewReview({ ...newReview, rating: star })} />))}
              </div>
            </div>
          </div>
          <form onSubmit={handleReviewSubmit} className="bg-white rounded-b-2xl p-4 shadow-md flex items-center relative">
            <textarea className="w-full bg-gray-100 rounded-full py-3 px-6 pr-12 resize-none focus:outline-none focus:ring-2 focus:ring-blue-500" placeholder="Write your review here . . ." rows="1" value={newReview.text} onChange={(e) => setNewReview({ ...newReview, text: e.target.value })}></textarea>
